-- Use GP item-level reorder/max stock from the primary inventory row (external_id = sku || '-')
-- instead of MAX() across all locations, which was picking up corrupt warehouse values.
