-- lead_time_days must come from item_supplier_reference for the selected
-- primary supplier (tenant_id + sku + supplier_external_id), not from products
-- or a single SKU-level default. supplier_lead_time_days is kept for reference
-- from the suppliers master only.
...
COMMENT ON VIEW vw_reorder_inputs IS
    'Reorder inputs per SKU. lead_time_days is from item_supplier_reference for the primary supplier row (sku + supplier_external_id), not a product-level default.';
